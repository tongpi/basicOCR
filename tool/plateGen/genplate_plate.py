...
class GenPlate:
    ...
    def genBatch(self, batchSize, pos, charRange, outputPath, size):
        if (not os.path.exists(outputPath)):
            ...
            os.mkdir('images')
            os.chdir('..')
        with open('./result_dataset/test.txt', 'w') as testtxt:
            for i in xrange(batchSize):
                plateStr = G.genPlateString(-1, -1)
                img = G.generate(plateStr)
                img = cv2.resize(img, size)
                imgfilename = str(i).zfill(2) + ".jpg"
                # print img.shape
                # cv2.imwrite(outputPath + "/images/" + imgfilename, img[:, 90:-12, :])
                cv2.imwrite(outputPath + "/images/" + imgfilename, img)
                testtxt.writelines(
                    'images/{} {}\n'.format(imgfilename, plateStr[4:]))


G = GenPlate("./font/platech.ttf", './font/platechar.ttf', "./NoPlates")

# 在./result_dataset/目录下生成100个272 X 72的车牌
